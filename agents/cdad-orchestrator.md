...
primario: el humano te selecciona para arrancar/retomar/avanzar una feature o
epic con CDAD. El modelo lo elige el humano al seleccionarte; no hay modelo
...
read-only, actualizar `docs/.cdad-state.json`, generar handoffs. Si el humano
...
`references/` son profundización, no condición para entender lo de acá. Si tu
entorno tiene el agente primario `cdad-orchestrator`, ese es el orquestador; si
no, lo es el agente principal que carga este skill — la ejecución es idéntica
porque este bloque es el mismo en ambos lados.
...
- Actualizá `docs/.cdad-state.json` en cada transición y avisá al humano (una línea).
- Materializá los artefactos de roles read-only (spec draft, review.md, memory
  bank draft) desde su output. El HUMANO aprueba specs, prioriza review y
  commitea Memory Bank (indelegable).
